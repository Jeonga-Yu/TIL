...
비트 연산자는 비트(bit)를 가지고 연산을 한다. 비트는 컴퓨터 기억장치의 최소 단위로 1비트는 0과 1만을 구분한다. 이진법의 1 자리다. 8비트는 1바이트라고 하며 1바이트로 나타낼 수 있는 수의 범위가 0부터 11111111 로 십진수로 바꾸면 0부터 255 까지 나타낼 수 있다.

비트 연산자에는 &(AND 연산), |(OR 연산), ^(XOR 연산), <<,>> (쉬프트 연산), ~(반전) 등이 있다.
...
아무리 최신의 Intel CPU 를 장착해도 컴퓨터는 0과 1만 처리한다.

2와 3같은 숫자도 처리하지 못하는데 어떻게 a, b, 가, 나 와 같은 수많은 문자를 처리할 수 있는가? 방법이있다. 이러한 문자들을 숫자에 대응시키는 것이다. 숫자에 대응시킨다면, 컴퓨터가 이 것이 숫자인지, 문자인지 어떻게 알까? 숫자를 문자형태로 사용하거나 숫자 형태로 사용하는 것이다.

문자를 저장하는 변수는 char 로 character 의 약자이다.
...
%d  는 변수의 값을 숫자(정수인 10진수)로 출력하라는 뜻

%c 변수의 값을 문자로 출력하라는 뜻

각 문자 마다 대응되는 숫자가 일정하게 정해져 있는데 미국 표준 학회 에서 정한 아스키 코드로 8비트 데이터를 이용하여 여러 문자에 번호를 붙였다. 표준을 정할 당시에는 7비트 만으로 충분하다고 생각했으나 IBM에서 더 많은 종류의 문자가 필요하게 되어 1비트를 추가한 확장된 아스키코드를 만들었다.

하지만 전 세계에 컴퓨터가 보급되고 더 많은 종류의 문자를 표현해야 했다.

그래서 유니코드라는 새로운 형식의 문자 체계를 도입한다. 유니코드는 한문자를 1에서 4바이트까지 다양한 길이로 처리한다. 이는 기존 아스키 코드의 체계를 유지하면서 새로운 문자들을 추가하기 위함이다.
...
*좋은 소스코드의 조건은 다른 사람이 이해하기 쉬운 소스코드 이다.

scanf를 통해 사용자로 부터 데이터를 받을 수 있다.

%c는 문자, %d는 정수, %f는 float, %lf는 double을 받는다.
